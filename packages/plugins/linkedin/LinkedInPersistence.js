...
class LinkedInPersistence {
    constructor() {
        this.outboxPath = path.join(process.cwd(), "sessions", "linkedin", "linkedin_pending_outbox.json");
    ...
    _addToOutbox(type, data) {
        ...
        logger.info(`📥 Saved LinkedIn outbox item [${type}] locally due to Oracle offline/unreachable state.`);
    }
    ...
    async flushOutbox() {
        ...
        logger.info(`📤 Attempting to flush ${outbox.length} pending LinkedIn outbox item(s) to Oracle Persistence API...`);
        try {
            const payload = { items: outbox };
            const res = await this._makeRequest("POST", "/api/linkedin/sync-outbox", payload, 10000);
            if (res && res.success) {
                logger.info(`✅ Successfully flushed ${res.processedCount || outbox.length} LinkedIn outbox item(s) to Oracle.`);
                this._writeOutbox([]);
                return true;
            }
        } catch (err) {
    ...
    async getDailyAppliedCount(dateStr) {
        ...
        try {
            const res = await this._makeRequest("GET", `/api/linkedin/daily-count?date=${encodeURIComponent(date)}`);
            if (res && res.success) {
                return res.count || 0;
            }
        } catch (err) {
            logger.warn(`⚠️ Failed to fetch Oracle LinkedIn daily count (${err.message}). Defaulting to 0.`);
        }
        return 0;
    }

    async isDuplicateJob(jobId, url) {
        // 1. Local outbox check
        const outbox = this._readOutbox();
        ...
        if (pendingInOutbox) {
            logger.info(`[Duplicate Check] Job ID ${jobId} found in local LinkedIn pending outbox. Treating as DUPLICATE.`);
            return true;
        }

        // 2. Query Oracle Persistence API
        try {
            const apiPath = `/api/linkedin/job-status?jobId=${encodeURIComponent(jobId)}&url=${encodeURIComponent(url || "")}`;
            ...
            logger.warn(`⚠️ Failed querying Oracle for LinkedIn job duplicate status: ${err.message}`);
        }
        return false;
    }
    ...
    async recordJob(job) {
        const payload = {
            ...
            url: job.url,
            salary: job.salary,
            ats: job.ats || job.applicationPortal || "linkedin",
            applicationPortal: job.applicationPortal || job.ats || "linkedin",
            isExternal: job.isExternal || job.applyType === 'EXTERNAL' || false
        };

        try {
            await this._makeRequest("POST", "/api/linkedin/record-job", payload);
        } catch (err) {
    ...
    async updateJobStatus(jobId, status, applied = null) {
        ...
        try {
            await this._makeRequest("POST", "/api/linkedin/update-status", payload);
        } catch (err) {
    ...
    async recordEvent(eventId, jobId, portal, eventType, payload) {
        ...
        try {
            await this._makeRequest("POST", "/api/linkedin/record-event", itemPayload);
        } catch (err) {
...
module.exports = new LinkedInPersistence();
