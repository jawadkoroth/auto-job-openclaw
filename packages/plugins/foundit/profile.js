...
module.exports = async function updateProfile(plugin, page) {
    const { logger } = plugin;
    logger.info("Foundit profile update sequence initiated.");
    ...
    await page.goto("https://www.foundit.in/seeker/profile", { waitUntil: "networkidle", timeout: 30000 }).catch(() => {});

    const resumePath = await resumeManager.getResumePath(plugin.name).catch(() => null);
    if (resumePath) {
        logger.info(`Uploading resume file to Foundit from: ${resumePath}`);
