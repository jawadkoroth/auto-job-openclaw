...
module.exports = async function updateProfile(plugin, page) {
    const { logger } = plugin;
    logger.info("Wellfound profile update sequence initiated.");
    ...
    await page.goto("https://wellfound.com/profile/edit", { waitUntil: "networkidle", timeout: 30000 }).catch(() => {});

    const resumePath = await resumeManager.getResumePath(plugin.name).catch(() => null);
    if (resumePath) {
        logger.info(`Uploading resume file to Wellfound from: ${resumePath}`);
