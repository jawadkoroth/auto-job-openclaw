...
module.exports = async function apply(plugin, page, job) {
    ...
    if (hasApplyBtn) {
        ...
        // Check if there is a final submission button in a modal
        const finalSubmitSelector = "button:has-text('Send Application'), button:has-text('Send application'), button:has-text('Submit Application'), button:has-text('Submit application')";
        if (await page.locator(finalSubmitSelector).count() > 0) {
            logger.info("Final application submission modal detected. Clicking final submit...");
        ...
        const successConfirmSelector = ".already-applied, button:has-text('Applied'), span:has-text('Applied'), :has-text('You have applied'), :has-text('Application sent'), :has-text('applied successfully')";
        ...
        if (isConfirmed) {
            ...
            logger.warn(`Click action performed but application confirmation not detected on Wellfound. Marking as CLICKED_UNVERIFIED.`);
